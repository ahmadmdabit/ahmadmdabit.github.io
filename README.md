...
- **`src/types`** — TypeScript interfaces for core data structures (`ResumeData`, `Project`, `Experience`, etc.) plus chat-specific types (`Chat.types.ts`: `DocumentSource`, `Message`, `UsageInfo`, `CompactionJob`, `SearchIndex`, etc.)
- **`src/constants`** — Feature-specific constants extracted for reuse:
  - `chat.ts` — LLM model configs (`AvailableLLMModels`, `LLMModel`, `LLMModelContextWindow`), compaction settings (`CompactionThresholdRatio`, `CompactionMaxRetries`, `CompactionRetryBaseDelayMs`), and `DocumentSources` (EN/TR markdown file paths)
- **`src/hooks`** — Custom React hooks encapsulating all chat subsystem logic, composed via a barrel export (`hooks/index.ts`):
  ...
  - `useCompaction` — Background conversation compaction with sequential queue, exponential backoff retry, and abort support
  - `useConversationHistory` — Full send/receive loop (streaming, tool calls, PII hydration, usage tracking, compaction triggering)
