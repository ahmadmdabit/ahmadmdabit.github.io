...
The following diagrams illustrate the key architectures and data flows. For the complete set of 15 detailed diagrams including component hierarchies, RAG workflow sequences, compaction workflows, routing structures, and security layers, see **[ARCHITECTURE-DIAGRAMS.md](ARCHITECTURE-DIAGRAMS.md)**.
...
- **SEO**: React 19 native `<title>`/`<meta>`/`<link>` hoisting (no react-helmet-async), JSON-LD structured data (WebSite, Person, BreadcrumbList), Open Graph + Twitter Cards, canonical URLs, path-prefixed hreflang (en/tr/x-default), robots.txt, sitemap.xml
- **Prerendering**: `gs-pro-cli --full-html` for SPA route prerendering (20 routes: 10 en + 10 tr) — lightweight alternative to puppeteer/react-snap
- **AI Chat**: `@heyputer/puter.js` for LLM inference with streaming, tool-calling, and abort controller support; automatic multi-model fallback via `src/lib/ModelFallback.ts` (pinned "known good" model, HTTP-200 error-body detection, free/paid interleaved attempt order)
...
- **`src/`** — Root level: `App.tsx` (layout with Outlet + loading state + footer with privacy/terms links), `main.tsx` (entry point with ThemeProvider), `router.tsx` (route config), `theme/index.ts` (MUI theme), `index.css` (global styles)
- **`src/types`** — TypeScript interfaces for core data structures (`ResumeData`, `Project`, `Experience`, etc.) plus chat-specific types (`Chat.types.ts`: `DocumentSource`, `Message`, `UsageInfo`, `CompactionJob`, `SearchIndex`, `FallbackModel`, `FallbackState`, etc.). `FallbackState` is **readonly at the type level** — its `pinnedIndex` is mutated only inside `ModelFallback.ts` (single cast site), making the ownership contract compiler-enforced.
- **`src/constants`** — Feature-specific constants extracted for reuse:
  - `chat.ts` — LLM model catalog (`AvailableLLMModels`: 20+ free/paid models with context window, `isFree` flag, and attempt `order`), default model constants (`LLMModel`, `LLMModelContextWindow`), compaction settings (`CompactionThresholdRatio`, `CompactionMaxRetries`, `CompactionRetryBaseDelayMs`), and `DocumentSources` (EN/TR markdown file paths)
- **`src/lib`** — Framework-agnostic chat runtime utilities:
  - `ModelFallback.ts` — Automatic model fallback library shared by the chat stream and compaction paths. Owns all `FallbackState` mutation (`pinModel` / `advancePinOnError`); exports shared abort detection (`isAbort`) and pure, unit-testable classifiers (`isModelError`, `isErrorShapedResponse`, `getAttemptOrder`)
- **`src/hooks`** — Custom React hooks encapsulating all chat subsystem logic, composed via a barrel export (`hooks/index.ts`):
  - `useDocumentIndex` — Loads, chunks, and indexes markdown documents (Resume, Projects, FAQ) into a bilingual MiniSearch index; also builds per-locale FAQ indexes
  - `useSearch` — General MiniSearch query with locale filtering + dedicated FAQ exact-match search against pre-built FAQ index
  - `useAutoScroll` — Smart auto-scroll with user-scroll-up detection and manual reset
  - `useCompaction` — Background conversation compaction with sequential queue, exponential backoff retry, abort support, and model-fallback-aware summarization (tracks the active fallback-pinned model, not static constants)
  - `useConversationHistory` — Full send/receive loop (streaming, tool calls, PII hydration, usage tracking, compaction triggering) with automatic multi-model fallback; exposes the live `currentModel`/`contextWindow` pair kept atomically in sync with the pinned model
  - `useKeyboardShortcuts` — Escape-to-close, Enter-to-send (IME-safe), auto-focus on mount
...
- **`src/molecules`** — Compositions of atoms forming complex components:
  ...
  - `ChatPopup` — Thin presentational wrapper for the AI-powered chat assistant; delegates all logic to the `src/hooks` subsystem (see [Chat System](#chat-system) below)
...
- **`src/pages`** — Page components rendered via Router Outlet (`HomePage`, `PrivacyPage` + `PrivacyContent`/`PrivacySections` shared blocks, `TermsOfServicePage`, `ErrorPage`)
- **`src/i18n`** — Internationalization configuration with locale JSON files (`en/translation.json`, `tr/translation.json`), schema validation, and asset hashing for cache busting. `locales.ts` provides `SUPPORTED_LOCALES`, `DEFAULT_LOCALE`, `isLocale()`, and `extractLocaleFromPath()` helpers.
- **`src/seo`** — SEO module with shared types (`types.ts`: `LooseT`), constants (`constants.ts`: `SITE_URL`, `OG_IMAGE`, `getOgImage()`), per-route meta builders (`pageMeta.ts`: `PAGE_META` registry, `buildFullMeta()`, `routeKeyFromPath()`), and JSON-LD structured data (`JsonLd.tsx`: WebSite + Person + BreadcrumbList schemas).
- **`public/robots.txt`** — Crawl rules referencing the sitemap.
- **`public/sitemap.xml`** — 20 URLs (10 en + 10 tr) with lastmod/changefreq/priority.
- **`scripts/prerender.mjs`** — SPA prerenderer using `gs-pro-cli --full-html` to capture `document.documentElement.outerHTML` for each route. Spawns `vite.js` directly (no `.cmd` shim) via `promisify-child-process` (eager-promise guards, SIGTERM + `taskkill` tree-kill shutdown). Parallel workers (CPU count − 1) with per-route watchdog, retry, mtime freshness guard, graceful SIGINT/SIGTERM shutdown, and a post-run CDP tab sweep. Configurable via `GS_PRO_CLI` env var.
- **`scripts/update-sitemap.mjs`** — Build step refreshing `<lastmod>` to today for all non-`yearly` URLs (yearly privacy/terms entries keep real content-change dates).
...
1. **Document Loading** — On chat open, `loadAndIndexDocuments()` fetches markdown documents for **both** locales in parallel (`Promise.all`):
   - Resume (EN + TR) — **Lazy PII hydration**: PII (email, phone, address) only included when user explicitly asks for contact info; otherwise stripped before LLM context
...
3. **Model Configuration & Automatic Fallback** — `src/constants/chat.ts` defines `AvailableLLMModels`, a catalog of free and paid models (each with context window, `isFree` flag, and attempt `order`). `src/lib/ModelFallback.ts` routes every LLM call through a fallback loop instead of hardcoding a single model:
   - **Attempt order** — pinned ("known good") model first, then remaining models in a 2-free / 1-paid interleaved cadence, each tier sorted by `order`
   - **Pinning** — a model is pinned on the first provable success (first stream chunk / resolved completion); the UI's `currentModel` + `contextWindow` pair syncs atomically at that moment via `onPin`
   - **Failure classification** — `isModelError()` pattern-matches puter.js plain-object throws (rate limiting, upstream unavailability, model-not-found); auth errors and malformed requests fail fast without retrying
   - **HTTP-200 error bodies** — puter.js _resolves_ (does not reject) some failed requests; `isErrorShapedResponse()` detects resolved error bodies and re-throws yielded ndjson error lines so they enter the same fallback path
   - **Streaming safety** — `chatStreamWithFallback` is an async generator that iterates inside the fallback loop: a model error before any visible text/tool output transparently advances to the next model; once output is visible, the error propagates (retrying would duplicate it)
   - **Un-pinning** — a model error on the pinned model advances the pin (`advancePinOnError`) so a persistently broken model is not retried first on every call
   - **Abort handling** — shared `isAbort()` checks the signal itself plus duck-typed `name === "AbortError"` (puter.js throws plain objects, not `Error` instances); user aborts never consume fallback attempts or the compaction retry queue
   - Chat and background compaction share one `FallbackState` (chat + compaction both use the "known good" model); its fields are readonly at the type level and mutated only inside the library
...
5. **Background Conversation Compaction** — **Non-blocking, queued compaction with exponential backoff**:
   - **Trigger**: Real token usage (prompt + input_cache_read from completed turns) exceeds 75% of the active model's context window (fallback-aware, not a static constant)
...
11. **Security & Privacy**:
    ...
    - **Explicit chat consent**: First message blocked until the user checks the Privacy Notice consent box (choice persisted in `sessionStorage` for the browsing session); consent gates the send button, Enter key, and global shortcuts
...
| :----------------------------------- | :---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Deterministic Context Compaction** | Built a non-blocking queue system (`queueMicrotask`) executing background history compaction via LLM summarizing. Triggers at 75% of the active model's context window (fallback-aware). Includes exponential backoff retry policies.                                                                                                                                                                                                     | Eliminates token overflows and preserves exact, high-fidelity professional state without arbitrary turn-truncation.                                                             |
| **Automatic Multi-Model Fallback**   | Built `src/lib/ModelFallback.ts`: an async-generator streaming fallback across a 20+ model catalog (pinned "known good" model, 2-free/1-paid interleaved attempt order). Handles puter.js resolved HTTP-200 error bodies, yielded ndjson error lines, plain-object throws (duck-typed abort detection via shared `isAbort`), and un-pins broken models. `FallbackState` is readonly at the type level — mutation confined to the library. | Survives per-model rate limits and outages transparently; compaction and chat share the last known-good model, and the UI's model/context-window badge stays atomically synced. |
| **Bilingual RAG Workflow**           | Implemented a single, unified `MiniSearch` index containing both English and Turkish documentation. Integrated a dual-mode `expandSearch` tool to trigger targeted FAQ or cross-locale search workflows.                                                                                                                                                                                                                                  | Enables instant, low-latency document and FAQ retrieval with seamless mid-conversation bilingual switching.                                                                     |
...
| **Chat Hook Extraction**             | Decomposed the monolithic `ChatPopup` molecule (~880 LOC) into 6 single-responsibility custom hooks (`useDocumentIndex`, `useSearch`, `useAutoScroll`, `useCompaction`, `useConversationHistory`, `useKeyboardShortcuts`) and a constants module (`src/constants/chat.ts`).                                                                                                                                                               | ChatPopup is now a thin presentational wrapper; all business logic is independently testable, reusable, and follows the Single Responsibility Principle.                        |
| **Content Security Policy**          | Added a strict CSP meta tag (`default-src 'self'`, explicit `connect-src` allowlist for Puter.ai and Getform/Forminit, `style-src` allowing Google Fonts) to mitigate XSS and data exfiltration vectors.                                                                                                                                                                                                                                  | Defense-in-depth security layer restricting resource loading to trusted origins.                                                                                                |
...
| ---------- | ---------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-09-04 | **Terms Route, Chat Consent & Privacy Rework** | **Terms of Service**: New localized `TermsOfServicePage` with full SEO wiring (route, page meta, JSON-LD, sitemap entries, prerender captures; sitemap/prerender grow 18→20 URLs). **Privacy**: Extracted shared `PrivacySections` reused by the privacy page and the contact-form `PrivacyAccordion` (replaces nested `Outlet` + deleted `ReplacePlaceholders` atom); Forminit rebrand note folded into section 4; notice rewritten to cover AI chat data flows. **Chat consent**: Explicit Privacy Notice checkbox (`sessionStorage`-persisted) gates send button, Enter key, and shortcuts before the first message. **Build fixes**: puter.js 2.6 `ChatOptionsWithSignal` intersection type; `prerender.mjs` migrated from `node:child_process` to `promisify-child-process` (direct `vite.js` spawn, eager-promise guards, SIGTERM + `taskkill` shutdown); new `scripts/update-sitemap.mjs` (skips `yearly` entries). **Assets**: `public/*.png` consolidated under `public/assets/imgs/` with matching hashed output paths and updated references.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| 2026-09-01 | **SEO & Prerendering**                         | **Routing**: Migrated to path-prefixed locale routing (`/en/skills`, `/tr/skills`) via `/:locale` param with `validateLocale` loader redirecting invalid locales to `/en`. Root `/` redirects to `/en`. **Per-route meta**: React 19 native `<title>`/`<meta>`/`<link>` hoisting (no react-helmet-async) via `src/seo/pageMeta.ts` registry with unique title/description/canonical/OG/Twitter Cards per route. **JSON-LD structured data**: WebSite + Person + BreadcrumbList schemas via `src/seo/JsonLd.tsx`. **hreflang**: Path-prefixed en/tr/x-default alternates. **Static assets**: `public/robots.txt` (crawl rules + sitemap ref), `public/sitemap.xml` (18 URLs: 9 en + 9 tr with lastmod/changefreq/priority). **Prerendering**: `scripts/prerender.mjs` using `gs-pro-cli --full-html` to capture `document.documentElement.outerHTML` for all 18 routes — lightweight alternative to puppeteer/react-snap. **i18n**: Locale extracted from URL path on init; `LanguageSelector` preserves current section on locale switch. **Navigation**: Manual `normalizePath` comparison for trailing-slash-insensitive active state; section-based animation key prevents replay on locale switch. **Shared modules**: `src/seo/types.ts` (LooseT), `src/seo/constants.ts` (SITE_URL, OG_IMAGE, getOgImage). **HTML**: Preconnect (Google Fonts), theme-color, noindex on 404/ErrorPage, `<base href="/">` on both index.html and 404.html.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| 2026-09-01 | **Automatic Multi-Model Fallback**             | New `src/lib/ModelFallback.ts`: automatic model fallback for both streaming chat and background compaction across the `AvailableLLMModels` catalog (20+ free/paid models in `src/constants/chat.ts`, default switched to `minimax/minimax-m3:free`, 1M context). Async-generator streaming fallback (pinned "known good" model first, 2-free/1-paid interleaved attempt order; transparent retry until visible output). Hardened against puter.js runtime quirks: resolved HTTP-200 error bodies, yielded ndjson error lines, plain-object (non-`Error`) throws, non-iterable stream responses, and providers that ignore `stream`. Pinned model un-pins on proven failure (`advancePinOnError`); shared `isAbort()` duck-typed abort detection replaces triplicated checks. `FallbackState` (new in `Chat.types.ts`) is readonly at the type level — mutation confined to the library. `useConversationHistory` now owns fallback state internally and exposes live `currentModel`/`contextWindow`; blank compaction summaries fail the job instead of silently replacing history; empty streaming placeholders are removed before error bubbles render.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| 2026-06-12 | **Chat Hook Extraction & CSP**                 | **Hook Decomposition**: Refactored `ChatPopup.tsx` from a monolithic class (~880 LOC) into a thin presentational wrapper delegating to 6 custom hooks: `useDocumentIndex` (document loading, chunking, bilingual MiniSearch indexing), `useSearch` (general + FAQ search), `useAutoScroll` (smart scroll with user-scroll detection), `useCompaction` (background queue-based compaction with exponential backoff), `useConversationHistory` (full send/receive streaming, tool calls, PII hydration, usage tracking), `useKeyboardShortcuts` (Escape, Enter IME-safe, auto-focus). Extracted chat constants (`src/constants/chat.ts`): LLM models, compaction thresholds, and `DocumentSources`. **Security**: Added Content-Security-Policy meta tag with strict origin allowlist (`default-src 'self'`, `connect-src` for Puter.ai/GetForm, `style-src` for Google Fonts). **Types**: Extended `CompactionJob` with `tokenUsage` field for accurate token reset after compaction.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| 2026-06-13 | **CSP Fix for 404**                            | Added `font-src data:` to CSP meta tag in 404.html to allow base64 fonts used by MUI Emotion; ensures consistent styling on GitHub Pages 404 page.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| 2026-07-01 | **Portfolio Overhaul & New Projects**          | **Master-Detail Projects Section**: Redesigned ProjectsSection with scrollable project list + hero detail card, virtual scrolling, and hero variant for ProjectCard. Wrapped ExperienceItem and SkillsSection in Cards with hover glow effects. Added page transition animations (fadeSlideIn) to HomePage. **Content Update**: Updated resume to reflect 10+ years experience, added SOLID Principles, added new projects (Personal Website RAG, Architectly, MCP CLI), removed old projects (RepoAIfy, AI Utils, E-Store). Updated certification year 2023→2025 with live reference link. Renamed certificate files (underscore→hyphen). Added DOCX resume files and updated PDF/TXT/MD for EN and TR. Synced EN/TR translation JSON with all changes. Removed asset hash query param from CV download URLs. **New Projects Added**: SevenZipRunner (.NET library for 7‑Zip invocation with platform‑aware resolution and named profiles), MCP Hub Web Interface (zero‑build Koa.js dashboard for monitoring MCP Hub servers), Semantic Tool Router CLI (TypeScript/Node.js CLI solving "fat agent" problem with offline indexing and just‑in‑time context injection). Renamed Semantic Tool Router CLI to Semantic Tool Router across all locales and sources.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| 2026-07-03 | **Turkish Resume Translation**                 | Completed full Turkish translation of resume content (section headers, contact info, technical skills, experience, project descriptions) in Resume‑TR.pdf and corresponding Resume‑TR.docx. Fixed translation of 'Strong' to 'Güçlü' and localized section headers and contact labels to Turkish.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
# Build (typecheck → hash locales → update sitemap → vite build → prerender)
yarn build
