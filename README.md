...
- **Prerendering**: `gs-pro-cli --full-html` for SPA route prerendering (18 routes: 9 en + 9 tr) — lightweight alternative to puppeteer/react-snap
...
- **`public/sitemap.xml`** — 18 URLs (9 en + 9 tr) with lastmod/changefreq/priority.
- **`scripts/prerender.mjs`** — SPA prerenderer using `gs-pro-cli --full-html` to capture `document.documentElement.outerHTML` for each route. Configurable via `GS_PRO_CLI` env var.
...
| **Content Security Policy**          | Added a strict CSP meta tag (`default-src 'self'`, explicit `connect-src` allowlist for Puter.ai and GetForm, `style-src` allowing Google Fonts) to mitigate XSS and data exfiltration vectors.                                                                                                                                                                                                                                           | Defense-in-depth security layer restricting resource loading to trusted origins.                                                                                                |
...
# Build (typecheck → hash locales → vite build → prerender)
yarn build
